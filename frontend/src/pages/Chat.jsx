...
const Chat = () => {
  ...
  const fetchConnections = async () => {
    try {
      ...
      const connectionList = res.data.map(conn => {
        ...
        // Assign live presence status randomly for SaaS mock realism
  ...
  const handleSendMessage = (e) => {
    ...
    if (attachedFile) {
      // Append file download hook info to backend message payload
    ...
    if (socket && activeUser) {
      ...
      // Fallback offline message logs (Guest Mode support)
      const offlineMsg = {
